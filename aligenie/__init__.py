...
CONFIG_SCHEMA = vol.Schema(
    {DOMAIN: vol.Schema({vol.Optional(EXPIRE_HOURS): cv.positive_int})},
...
async def async_create_refresh_token78(
    ...
    _LOGGER.info("access token expiration: %d hours 78", _expire_hours)
    return async_do_create_refresh_token(**locals())

async def async_create_refresh_token2402(
        user: models.User,
        client_id: Optional[str] = None,
        client_name: Optional[str] = None,
        client_icon: Optional[str] = None,
...
def async_do_create_refresh_token(**kwargs):
    # if kwargs.get("access_token_expiration") == ACCESS_TOKEN_EXPIRATION:
    kwargs["access_token_expiration"] = timedelta(hours=_expire_hours)
    _LOGGER.info("Access token expiration: %d hours", _expire_hours)
    """Create a new token for a user."""
    ...
    kwargs["user"].refresh_tokens[refresh_token.id] = refresh_token

    _hass.auth._store._async_schedule_save()
    return refresh_token


def get_info_from_aligenie():
    _places = json.loads(
        urlopen("https://open.bot.tmall.com/oauth/api/placelist").read().decode("utf-8")
    )["data"]
    _aliases = json.loads(
        urlopen("https://open.bot.tmall.com/oauth/api/aliaslist").read().decode("utf-8")
    )["data"]
    return (_places, _aliases)
...
def errorResult(errorCode, messsage=None):
    ...
    return {
        "errorCode": errorCode,
        "message": messsage if messsage else messages[errorCode],
    }
...
async def handleRequest(data):
    ...
    token = _hass.auth.async_validate_access_token(payload["accessToken"])
    if token is not None:
        namespace = header["namespace"]
        _LOGGER.info(f'namespace:{namespace}')
        ...
        devices.append(
            {
                ...
                "properties": [prop],
                "actions": ALL_ACTIONS + ["Query"]
                if action == "QueryPowerState"
                else ["Query", action],
...
async def controlDevice(action, payload):
    ...
    if domain in TRANSLATIONS.keys():
        ...
        if callable(translation):
            ...
            _LOGGER.info("Service:%s, Content:%s" % (service, content))
            data.update(content)
            if "adb_command" == service:
                domain = "androidtv"
        else:
            service = translation
    else:
        service = getControlService(action)
    _LOGGER.info("Domain:%s, Service:%s, Data:%s" % (domain, service, data))

    # with AsyncTrackStates(_hass) as changed_states:
    result = await _hass.services.async_call(
...
def queryDevice(name, payload):
    ...
    if payload["deviceType"] == "sensor":
        ...
        for state in states:
            attributes = state.attributes
            if state.entity_id.startswith("group.") and (
                attributes["friendly_name"] == deviceId
                or attributes.get("hagenie_zone") == deviceId
            ):
        ...
        for state in states:
            ...
            if entity_id.startswith("sensor.") and (
                ...
                or attributes["friendly_name"].startswith(deviceId)
                or attributes.get("hagenie_zone") == deviceId
            ):
        ...
        if state is not None or state.state != "unavailable":
            return {
                "name": "powerstate",
                "value": "off" if state.state == "off" else "on",
            }
...
MEDIA_PLAYER_DICT = {
    ("media_player.living_room_tv", "有线模式"): ("select_source", {"source": "CBL/SAT"}),
    ("media_player.living_room_tv", "电影模式"): (
        "select_source",
        {"source": "Media Player"},
    ),
    ("media_player.sony_bravia_tv", "电影模式"): ("select_source", {"source": "HDMI 2"}),
    ("media_player.living_room_tv", "游戏模式"): ("select_source", {"source": "Game"}),
    ("media_player.living_room_tv", "任天堂模式"): ("select_source", {"source": "AUX2"}),
    ("media_player.living_room_tv", "腾讯视频模式"): (
        "adb_command",
        {
            "entity_id": "media_player.living_room_box",
            "command": "monkey -p 'com.ktcp.video' -v 1 &> /dev/null",
        },
    ),
    ("media_player.sony_bravia_tv", "腾讯视频模式"): (
        "adb_command",
        {
    ...
    ("media_player.living_room_tv", "爱奇艺视频模式"): (
        "adb_command",
        {
            "entity_id": "media_player.living_room_box",
            "command": "monkey -p 'com.gitvdemo.video' -v 1 &> /dev/null",
        },
    ),
    ("media_player.sony_bravia_tv", "爱奇艺视频模式"): (
        "adb_command",
        {
            "entity_id": "media_player.main_bedroom_tv",
            "command": "monkey -p 'com.gitvdemo.video' -v 1 &> /dev/null",
    ...
    ("media_player.living_room_tv", "优酷视频模式"): (
        "adb_command",
        {
            "entity_id": "media_player.living_room_box",
            "command": "monkey -p 'com.cibn.tv' -v 1 &> /dev/null",
        },
    ),
    ("media_player.sony_bravia_tv", "优酷视频模式"): (
        "adb_command",
        {
            "entity_id": "media_player.main_bedroom_tv",
            "command": "monkey -p 'com.cibn.tv' -v 1 &> /dev/null",
...
TRANSLATIONS = {
    "cover": {"TurnOn": "open_cover", "TurnOff": "close_cover"},
    "vacuum": {"TurnOn": "start", "TurnOff": "return_to_base"},
    "media_player": {
        "TurnOn": "turn_on",
        "TurnOff": "turn_off",
        "Pause": "media_pause",
        "Continue": "media_play",
        "Play": "media_play",
        "AdjustUpVolume": "volume_up",
        "AdjustDownVolume": "volume_down",
        "SetMode": lambda state, payload: mapping(
            MEDIA_PLAYER_DICT, (state.entity_id, payload["value"])
        ),
    },
    "light": {
        "TurnOn": "turn_on",
        "TurnOff": "turn_off",
        "SetBrightness": lambda state, payload: (
            "turn_on",
            {"brightness_pct": mapping({"max": 100, "min": 1}, payload["value"])},
        ),
        "AdjustUpBrightness": lambda state, payload: (
            "turn_on",
            {
                "brightness_pct": min(
                    state.attributes["brightness"] * 100 // 255 + int(payload["value"]),
                    100,
                )
            },
        ),
        "AdjustDownBrightness": lambda state, payload: (
            "turn_on",
            {
                "brightness_pct": max(
                    state.attributes["brightness"] * 100 // 255 - int(payload["value"]),
        ...
        "SetColor": lambda state, payload: (
            "turn_on",
            {"color_name": payload["value"]},
        ),
    },
    "climate": {
        "TurnOn": "turn_on",
        "TurnOff": "turn_off",
        "SetTemperature": lambda state, payload: (
            "set_temperature",
            {"temperature": int(payload["value"])},
        ),
        "AdjustUpTemperature": lambda state, payload: (
            "set_temperature",
            {
                "temperature": min(
                    state.attributes["temperature"] + int(payload["value"]),
                    state.attributes["max_temp"],
                )
            },
        ),
        "AdjustDownTemperature": lambda state, payload: (
            "set_temperature",
            {
                "temperature": max(
                    state.attributes["temperature"] - int(payload["value"]),
        ...
        "SetMode": lambda state, payload: (
            "set_hvac_mode",
            {
                "hvac_mode": mapping(
                    {"cold": "cool", "dehumidification": "dry"}, payload["value"]
                )
            },
        ),
        "SetWindSpeed": lambda state, payload: (
            "set_fan_mode",
            {"fan_mode": mapping({"max": "4", "min": "1"}, payload["value"])},
        ),
    },
    "fan": {
        "TurnOn": "turn_on",
        ...
        "SetWindSpeed": lambda state, payload: (
            "set_speed",
            {"speed": mapping({"max": "high", "min": "low"}, payload["value"])},
        ),
...
def groupsAttributes(states):
    ...
    for state in states:
        ...
        if (
            group_entity_id.startswith("group.")
            and not group_entity_id.startswith("group.all_")
            and group_entity_id != "group.default_view"
        ):
